...
def quantization_error(
    ...
    return (dequantized_error - tensor).abs().square().mean()


def plot_matrix(cls, tensor, ax, title, vmin=0, vmax=1, cmap=None):
    """Plot heatmap of tensors using seaborn"""
    sns.heatmap(
        tensor.cpu().numpy(),
        ax=ax,
        vmin=vmin,
...
def plot_quantization_errors(
    ...
    plot_matrix(
        original_tensor, axes[0], "Original Tensor", cmap=ListedColormap(["white"])
    )
